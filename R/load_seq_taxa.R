...
load_seq_taxa <- function(date = "230212"){
  ...
  seqtab <- data.frame(fread(paste("../../data/raw_data/microbes/seq.table_", date, ".csv"), header = TRUE, check.names = FALSE), row.names = 1)

  # Load sequence identity table
  taxa <- read.csv(paste("../../data/raw_data/microbes/taxa_", date, ".csv"), header = TRUE, check.names = FALSE, row.names = 1)
